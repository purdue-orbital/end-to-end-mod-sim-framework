...
% time step sensor integration
if x == 1
    time_int = 0;
    time_step = 1;
    time_max = input('How many pointer files are present?\n');
    color_shift = 255 / time_max;
    for pat_num = 1:time_max
        ref_dis = importdata('sensorpattern_' + string(pat_num) + '.Pattern').data;
        sensor = facility.Children.New('eSensor', 'Sensor_' + string(pat_num) + 's');
        sensor.Graphics.Color = blue * (256^2) + red;
...
% height step sensor integration
% pattern files must have consecuvtive heights starting at z = 0 km
% hard code ref_dis for specific pattern heights
if x == 0
    ref_dis = 0;
    max_height = input('How many pattern files are present?\n');
    color_shift = 255 / max_height;
